...
- **目的:** 自身のスキル、成果物（GitHub）、発信（Blog・Zenn）を一元管理し、技術力をアピールする。
- **主要機能:**
  - GitHub リポジトリ（Pinned Items / Star 数）の自動表示
  - Blog・Zenn記事のRSS索引と、Zenn最新記事のHome表示
  - 静的サイト生成（SSG）による高速表示
...
| **Data Fetch**   | **RSS**              | Blog・ZennのRSS Feed                                       |
...
│   │   ├── blog.ts      # Blog RSSの取得・正規化
│   │   └── zenn.ts      # Zenn RSSの取得・正規化
│   ├── pages/           # ルーティング
...
### 4.2. 記事データ取得 (`src/lib/blog.ts`, `src/lib/zenn.ts`)

APIキー不要のRSSフィードを使用します。`/writing/`では両方を統合して日付降順に並べ、HomeではZennの最新3件だけを表示します。

- **Endpoints:** `https://blog.kinn-kinn.com/rss.xml`, `https://zenn.dev/kinn00kinn/feed`
- **Library:** `rss-parser`
